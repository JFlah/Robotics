...
//		System.out.println("stop called");
	...
	public void travelArc(double radius, double distance,
		...
		//waitForActiveMove();  //NEED FOR THIS AS WELL?
		if (radius == Double.POSITIVE_INFINITY
				|| radius == Double.NEGATIVE_INFINITY) {
	...
	public void steer(double turnRate) {
		...
		movementStart();  //this is new postion
		_left.startSynchronization();
		if (_parity > 0)
			_outside.forward();
		else
		...
		if (_parity * _steerRatio > 0)
			...
			_inside.backward();
		_left.endSynchronization();
		movementActive();  //this is new
	}

	
	public boolean checkDone(double x, double y, double x2, double y2) {
		if (x < x2 + 2.0 && x > x2 - 2.0) {
			if (y < y2 + 2.0 && y > y2 - 2.0) {
				return true;
			}
		}
		return false;
